...
const std::size_t kPortAsyncReadSize = 3U;    // TODO test increasing size
...
class Port : public std::enable_shared_from_this<Port>
{
    public:
        Port(void);
        Port(const std::string& device);
        Port(Port &port)                  = delete;
        Port(Port &&port)                 = delete;
        Port &operator=(const Port &port) = delete;
        Port &operator=(Port &&port)      = delete;
        void Open(const uint32_t baud_rate);
        void Open(const std::string& device, const uint32_t baud_rate);
